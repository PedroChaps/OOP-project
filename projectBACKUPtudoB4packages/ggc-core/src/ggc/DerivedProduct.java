...
public class DerivedProduct extends Product {
    ...
    public String toString(){
        return super.toString() + "|" + _alpha + "|" + _recipe;
    }

    public Breakdown acceptBreakdown(ProductVisitor visitor, Partner p, int amount){
        return visitor.sellBatches(this, p, amount);
    }
    ...
    public boolean checkAvailability(int amount) throws NotEnoughQuantityException{
        if (getStock() >= amount)
            return true;
        ...
        for (Ingredient i : ingredients)
            if(i.getProduct().getStock() < amountLeft * i.getAmount())
                throw new NotEnoughQuantityException(i.getProduct(), amountLeft *i.getAmount(), i.getProduct().getStock());
        return true;
    }
