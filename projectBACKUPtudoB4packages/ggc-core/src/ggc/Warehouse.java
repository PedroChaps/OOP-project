 ...
 // private HashMap<Product,HashSet<Batch>> _batches = new HashMap<>();
  ...
  private void updateProductInfo(String productID, double price, int stock){
    ...
    if (product.getStock() == 0)
      product.notifyObservers(new New(getDeliveryMethod(), product, price));
    ...
    if (price < minPrice) {
      ...
      product.notifyObservers(new Bargain(getDeliveryMethod(), product, price));
    }
  ...
  void importFile1(String txtfile) throws IOException, BadEntryException{
    BufferedReader in = new BufferedReader(new StringReader(txtfile));
    String line;
    String[] splitted;
    ...
    try{
      ...
      if(amount<=product.getStock()) {
        ...
        while (amountLeft != 0) { //FIXME quando criamos temos de os por numa batch mas eles podem ter todos precos diferentes ?? por causa do preco dos produtos usados, como fazemos? Criamos uma batch por produto
          maxPricePerUnit = product.acceptSale(new SaleVisitor(this));
      ...
      throw new LowOnStockException(e.getId(),e.getRequested(),e.getAvailable());
    }
    catch (UnsupportedOperationException e){}//TODO
    }
  ...
  public void updateProductMaxPrice(Product product, double maxPrice){
  ...
  public List<Transaction> processShowAcquisitionsByPartner(String partnerID) throws UndefinedPartnerKeyException{
    ...
    Partner p = _partners.get(partnerID.toLowerCase());
      ...
      if (transactions != null)
        for (Transaction t : transactions){
          t.acceptAcquisitionVisitor(new AcquisitionVisitor(this,acquisitions));
        }

      return acquisitions;
  }

  public List<String> processShowBreakdownsAndSalesByPartner(String partnerID) throws UndefinedPartnerKeyException{
    ...
    if (transactions != null)
      for (Transaction t : transactions){
        t.acceptSalesAndBreakDownVisitor(new SalesAndBreakDownVisitor(this,acquisitions, _date));
      }
  ...
  public void processRegisterAcquisitionNewProduct(String partnerID, String productID, double price, int amount) throws UndefinedPartnerKeyException{
    Partner partner = _partners.get(partnerID.toLowerCase());
    Product product = _products.get(productID);

    if (partner == null)
      throw new UndefinedPartnerKeyException(partnerID);
    Acquisition a = new Acquisition(getTransactionsID(),amount,price*amount,partner,product);
  ...
  public String craftExtendedRecipe(DerivedProduct pr, int breakdownAmount){
    ...
    for (Ingredient i : ingredients){
      ...
      extendedRecipe += productID + ":" + i.getAmount()*breakdownAmount + ":" + Math.round(findLowestPriceOfProduct(i.getProduct())*i.getAmount()*breakdownAmount) + "#";
    }
