...
public class Warehouse implements Serializable {
  ...
  public void processChangePartnerName(String partnerID, String newName) throws UndefinedPartnerKeyException{
    p = _partners.get(partnerID.toLowerCase());
    if (p == null) throw UndefinedPartnerKeyException(partnerID);
  ...
  public Transaction processGetHighestPriceTransaction(){
    double maxPrice = 0;
    Transaction maxPriceTransaction;
    
    for (Partner p: _partners){
      transactions = _transactions.get(p);
      if (transactions != null)
        for (Transaction t: transactions){
    ...
    return maxPriceTransaction;  
  }
  
  
  public Sale processGetHighestPriceSale(){
    double maxPrice = 0;
    Sale maxPriceSale;
    ...
    for (Partner p: _partners){
      ...
      if (transactions != null)
        for (Transaction t: transactions){
          ...
          if (t.getPrice() > maxPrice){
            try{
              maxPriceSale = t.acceptFindHighestSale(new HighestSaleVisitor(this));
              maxPrice = t.getPrice();
            }
            catch (UnsupportedOperationException e){}
          }
